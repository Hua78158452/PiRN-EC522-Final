...
if __name__ == "__main__":
    ...
    train_batch_size, test_batch_size = 2, 4
    num_epochs = 80
    all_T = 100000
    save_dir = "/projectnb/ec522/students/huatong/PiRN-main/assets/checkpoint/"
    print("save_dir:", save_dir)
    print("Directory to create:", os.path.dirname(save_dir))

    # 确保路径不为空
    if not os.path.dirname(save_dir):
        raise ValueError("Path is empty. Please check the save_dir variable.")

    # if not os.path.isfile(save_dir):
    #     os.makedirs(os.path.dirname(save_dir), exist_ok=True)
    if not os.path.isfile(save_dir + "/training_samples"):
        os.makedirs(save_dir + "/training_samples", exist_ok=True)

    dataset = CustomImageFolderDataset(root="/projectnb/ec522/students/huatong/gt/",
    ...
    optimizer, lr_scheduler = optim.configure_optimizers(net, adaface_model, head,
                                                         lr=1e-4, momentum=0.9, lr_milestones=[6, 15, 25, 50],
                                                         lr_gamma=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_id, train_data in tqdm(enumerate(dataloader)):
            ...
            if not (batch_id % 200):
                fopen.write(
                    'Epoch: {}, Iteration: {}, Loss: {:.3f}, Adaface_Loss: {:.3f}, Rec_Loss1: {:.3f}, Rec_loss2: {:.3f}, Rec_loss3: {:.3f}'.format(
                        epoch, batch_id, loss, AdaFace_loss, Rec_Loss1, Rec_Loss2, Rec_Loss3))
                print(
                    'Epoch: {}, Iteration: {}, Loss: {:.3f}, Adaface_Loss: {:.3f}, Rec_Loss1: {:.3f}, Rec_loss2: {:.3f}, Rec_loss3: {:.3f}'.format(
                        epoch, batch_id, loss, AdaFace_loss, Rec_Loss1, Rec_Loss2, Rec_Loss3))
                fopen.flush()
