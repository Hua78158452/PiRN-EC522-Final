...
class BaseSampler:
    ...
    def load_model(self, model, ckpt_path=None):
        if not ckpt_path is None:
            ...
            ckpt = torch.load(ckpt_path, map_location=f"cuda:{self.rank}")
            if "model_net" in ckpt.keys():
                # print(ckpt['model_net'].keys())
                # print(ckpt['model_adaface'])
                # print(model)
                util_net.reload_model(model, ckpt['model_net'])
            # if "model_net" in ckpt.keys():
            #      util_net.reload_model(model, ckpt["model_net"])
            else:
        ...
        if need_restoration:
            with torch.no_grad():
                if model_kwargs_ir is None:
                    im_hq = self.model_ir(y0)[1]
                else:
                    im_hq = self.model_ir(y0, **model_kwargs_ir)[1]
        else:
            im_hq = y0
