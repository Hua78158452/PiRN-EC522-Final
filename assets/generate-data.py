...
mode = "train"
path = "E:/EC522-finalproject/FFHQ/Places"

image_dirs = os.listdir(os.path.join(path, mode))
...
for img_dir in tqdm(image_dirs):
    print(path)
    path_img = os.path.join(path, mode, img_dir)
    print(path_img)
    # os.mkdir(path + "/gt/" + img_dir)
    # os.mkdir(path + "/noise/" + img_dir)
    # os.mkdir(path + "/sim/" + img_dir)
    # os.mkdir(os.path.join(path, "gt", img_dir))
    # os.mkdir(os.path.join(path, "noise", img_dir))
    # os.mkdir(os.path.join(path, "sim", img_dir))
    os.makedirs(os.path.join(path, "gt", img_dir))
    os.makedirs(os.path.join(path, "noise", img_dir))
    os.makedirs(os.path.join(path, "sim", img_dir))
